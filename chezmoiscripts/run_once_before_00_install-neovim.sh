...
case $ARCH in
  ...
  aarch64 | arm64)
    ...
    # Note: As of 0.10.x, there are official ARM64 tarballs
      ...
      if wget -q --show-progress "$NVIM_URL" -O nvim.appimage; then
        ...
        if ./nvim.appimage --appimage-extract &> /dev/null; then
          ...
          if ! "$NVIM_BIN_DIR/nvim" --version &> /dev/null; then
            ...
            echo ""
            echo "Or build from source (see instructions below)"
            rm -f "$NVIM_BIN_DIR/nvim"
            rm -rf "$TMP_DIR"
            exit 1
          fi
  ...
  *)
    ...
    echo "  sudo apt-get install ninja-build gettext cmake unzip curl build-essential"
    echo "  git clone https://github.com/neovim/neovim"
    echo "  cd neovim && git checkout stable"
    echo "  make CMAKE_BUILD_TYPE=RelWithDebInfo CMAKE_INSTALL_PREFIX=$HOME/.local"
    echo "  make install"
    rm -rf "$TMP_DIR"
    exit 1
    ;;
